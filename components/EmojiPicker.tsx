...
export default function EmojiPicker({ onSelect, mode, onClose }: EmojiPickerProps) {
    ...
    return (
      <div className="relative">
        ...
        >
          {QUICK_EMOJIS.map((emoji) => (
            <button
              ...
              className="w-7 h-7 flex items-center justify-center text-base rounded-md transition-all duration-fast active:scale-95 select-none"
              style={{
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="absolute -top-2 -right-2 w-5 h-5 flex items-center justify-center rounded-full bg-red-500 text-white text-xs shadow-lg hover:bg-red-600 transition-all duration-fast"
            title="Close"
          >
            ×
          </button>
        ...
        <div className="p-1.5 grid grid-cols-6 gap-0.5 max-h-44 overflow-y-auto">
          {EXTENDED_EMOJIS[activeCategory].map((emoji, index) => (
            <button
              key={index}
              onClick={() => onSelect(emoji)}
              className="w-7 h-7 flex items-center justify-center text-sm rounded-md transition-all duration-fast active:scale-95 select-none"
              style={{
                background: 'var(--surface)',
                minWidth: '1.75rem',
                minHeight: '1.75rem',
