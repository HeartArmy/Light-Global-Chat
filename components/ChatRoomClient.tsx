...
export default function ChatRoomClient() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    Promise.all([
      fetch('/api/messages?limit=100').then(res => res.json()),
    ...
    <div className="h-screen flex flex-col" style={{ background: 'var(--background)' }}>
      ...
      >
        ...
        <div className="flex items-center gap-1 md:gap-2.5">
          {/* Social Links */}
          <div className="flex items-center gap-0.5 md:gap-1 opacity-70 hover:opacity-100 transition-opacity">
            <a
              ...
              onClick={(event) =>
                handleExternalLinkClick(
                  event,
                  "This will open the developer's personal website in a new tab. Continue?"
                )
              }
              className="p-1 rounded-sm transition-all duration-fast hover:scale-110 text-xs md:text-sm"
            ...
            >
              <svg width="12" height="12" className="md:w-3.5 md:h-3.5" viewBox="0 0 16 16" fill="currentColor">
